...
describe("useGeneratePrompt", () => {
	...
	describe("Basic prompt generation", () => {
		...
		it("should generate prompt with multiple variable replacements", () => {
			...
			expect(mockSetGeneratedPrompt).toHaveBeenCalledWith(
				"Hello Alice, you are 30 years old and work as a Developer.",
			);
		});

		it("should replace duplicate variables correctly", () => {
			const originalPrompt =
				"Hello ${name}, nice to meet you ${name}! Your name ${name} is beautiful.";
			...
			expect(mockSetGeneratedPrompt).toHaveBeenCalledWith(
				"Hello Sarah, nice to meet you Sarah! Your name Sarah is beautiful.",
			);
		});

		it("should handle variables with underscores and numbers", () => {
			const originalPrompt =
				"User ${user_id} has ${item_count} items and ${total_amount} dollars.";
			const variableValues = {
				user_id: "12345",
				item_count: "42",
				total_amount: "99.99",
			...
			expect(mockSetGeneratedPrompt).toHaveBeenCalledWith(
				"User 12345 has 42 items and 99.99 dollars.",
			);
		});
	});
	...
	describe("Handling missing values", () => {
		...
		it("should handle mixed filled and empty values", () => {
			...
			expect(mockSetGeneratedPrompt).toHaveBeenCalledWith(
				"Name: Bob, Age: ${age}, City: New York",
			);
		});
	...
	describe("Edge cases", () => {
		...
		it("should handle prompt with no variables", () => {
			...
			expect(mockSetGeneratedPrompt).toHaveBeenCalledWith(
				"This is a plain text prompt with no variables.",
			);
		});
		...
		it("should handle special characters in variable values", () => {
			...
			expect(mockSetGeneratedPrompt).toHaveBeenCalledWith(
				"Message: Hello! How are you? 😊, Email: user@example.com",
			);
		});
	...
	describe("Complex scenarios", () => {
		it("should handle prompt with variables that have similar names", () => {
			const originalPrompt =
				"Values: $\{value}, $\{value_1}, $\{value_11}, $\{value_2}";
			const variableValues = {
				value: "A",
				value_1: "B",
				value_11: "C",
